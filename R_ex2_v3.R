...
## SCENARIO WITH METRONOMIC THERAPY: FIXED DOSE EVERY 2 DAYS

## Fitness definition

drug_eff1 <- 0.01  # first dose
drug_eff2 <- 0.05  # next doses

fit_cells3 <-data.frame(Genotype = c("WT", "S", "R", "S, R"),
                        Fitness = c(wt_fitness,                         #WT
                                paste0("if (T>50 & T<70)", 
                                         drug_eff1, "*(", sens_fitness,")",
                                       "; else if (T>72 & T<82) ", 
...
fit_cells4 <-data.frame(Genotype = c("WT", "S", "R", "S, R"),
                        Fitness = c(wt_fitness,                                              #WT
                                    paste0("if (T>50 & N>500) ", drug_eff, "* (", sens_fitness,")",
                                           "; else ", sens_fitness, ";"),                    #S
                                    "0",                                                     #R
                                    res_fitness),                                            #S,R
                        stringsAsFactors = FALSE)
